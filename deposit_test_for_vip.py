...
if __name__ == "__main__":
    ...
    label_id_list=['58015','58016','58013','58014']
    current_dir=os.path.dirname(__file__)
    yaml_path=os.path.join(current_dir,"config.yaml")
    with open(yaml_path,"r",encoding="utf-8") as f:
        config=yaml.safe_load(f)
    testing_account=config.get("VIP_testing_account")
    try:
        backend=Backend(credential_be)
        if backend.token:
            logging.info("backend class有成功運作")
            for account,label in zip(testing_account,label_id_list):
                credential_fe = {
                "username": account,
                "password": password
                 }
                customerId=backend.search_customerid(account)
                is_success=backend.player_rank(customerId,account,int(label))
                if is_success:
                    frontend = Frontend(credential_fe)
                    if frontend.token:
                        frontend.deposit_QAD(account)
                        time.sleep(1) 
