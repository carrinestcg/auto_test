...
def api_input_user_info():
    ...
    if require_types_raw is None:
        require_types_raw = [data.get("requireType", data.get("type", 1))]
    if not isinstance(require_types_raw, (list, tuple)):
        require_types_raw = [require_types_raw]

    require_types = []
    for item in require_types_raw:
        try:
            require_types.append(int(item))
    ...
    for require_type in require_types:
        ...
        if value is not None and require_type in (1, 2, 4, 5, 6, 10, 11, 12, 13, 14, 15, 16, 17):
            value = str(value)
        ...
        for field_key in value_fields.values():
            item[field_key] = None
        item[value_fields[require_type]] = value
        results.append(item)
    ...
    if len(results) == 1:
        ...
        response = {
            ...
            "requireType": single["requireType"],
            "verifyType": single["requireType"],
            "results": results,
        }
        for field_key in value_fields.values():
            response[field_key] = single.get(field_key)
        return jsonify(response)

    response = {
        "success": overall_success,
        "message": f"共 {len(results)} 項：成功 {success_count} 項，失敗 {len(results) - success_count} 項",
...
def api_Manual_Sign():
    ...
    requireType=data.get("requireType", "B") 
    password = (data.get("password") or "").strip() or "123qwe"
    ...
    if not requireType:
        return jsonify({"success": False, "message": "請提供 requireType"}), 400

    result = MANUAL_SIGN.main(
        username,
        promotion_id,
        requireType,
        password=password,
    ...
    if isinstance(result, dict):
        if result.get("success"):
            return jsonify({"success": True, "message": result.get("message", "Trigger API Successfully")}), 200
        return jsonify({"success": False, "message": result.get("message", "Trigger API Failed"), "data": result}), 200
